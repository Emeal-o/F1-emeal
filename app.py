...
with tabs[1]:
    ...
    try:
        ...
        # Fallback bar chart using HTML when Plotly fails on Python 3.14
        st.markdown('<div style="background:#0d0d0d;border:1px solid #1a1a1a;padding:16px">', unsafe_allow_html=True)
        max_p = drv_std[0]["pts"] if drv_std else 1
        for d in drv_std[:10]:
            col = tc(d.get("team",""))
            pct = round(d["pts"]/max_p*100)
            name = d["name"].split()[-1]
            st.markdown(f'''<div style="display:flex;align-items:center;gap:10px;margin:5px 0">
              <div style="min-width:80px;font-size:12px;font-weight:700;color:{col}">{name}</div>
              <div style="flex:1;background:#1a1a1a;height:20px">
                <div style="width:{pct}%;background:{col};height:100%;display:flex;align-items:center;padding-left:6px">
                  <span style="font-size:11px;font-weight:700;color:#000">{int(d["pts"])}</span>
                </div>
              </div>
        ...
        if ff1_laps and ff1_laps.laps is not None and len(ff1_laps.laps) > 0:
            ...
            if sel_drvs:
                ...
                try:
                    ...
                    st.markdown('<div class="empty">Lap chart unavailable on this Python version</div>', unsafe_allow_html=True)
                ...
                if "Compound" in laps_df.columns:
                    ...
                    try:
                        ...
                        st.markdown('<div class="empty">Stint chart unavailable on this Python version</div>', unsafe_allow_html=True)
